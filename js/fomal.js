...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | JackLee")[0];
}
...
$.ajax({
  ...
  data: {
    key: 'SWOBZ-66SL7-E6FXC-HADM3-X4FAV-SEFIW',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(102.854553, 24.854019, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
  let pos = ipLoacation.result.ad_info.nation;
  let ip;
  let posdesc;
  console.log("1", dist)
  console.log("国家", pos)
  console.log("ip", ipLoacation.result.ip)
  //根据国家、省份、城市信息自定义欢迎语
...
function createtime1() {
  var grt = new Date("03/05/2025 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by JackLee %c 你正在访问JackLeeの小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| JackLee") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('JackLee的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  var dis = Math.trunc(((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("03/05/2025 00:00:00");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='/assets/less_ico.png' title='今年发大财~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' src='/assets/less_ico.png' title='下班了就该开开心心地玩耍~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://browser6.qhimg.com/dm/1506_848_/t0154340551e2325201.jpg);
    --darkmode-bg:url(https://browser6.qhimg.com/dm/1506_848_/t0154340551e2325201.jpg);
    --mobileday-bg: url(https://browser6.qhimg.com/dm/1506_848_/t0154340551e2325201.jpg);
    --mobilenight-bg: url(https://browser6.qhimg.com/dm/1506_848_/t0154340551e2325201.jpg);
  }`;
}
// 切换背景主函数
function changeBg(s) {
